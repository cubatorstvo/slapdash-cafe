...
func run() -> void:
	...
	var normal_bounds := Kitchen.sausage_pick_bounds("table")
	var catch_bounds := Kitchen.sausage_pick_bounds("slipped")
	check(catch_bounds.size.x > normal_bounds.size.x and catch_bounds.size.y > normal_bounds.size.y and catch_bounds.size.z > normal_bounds.size.z, "Slipped sausage gets a larger catch hitbox")
	check(Kitchen.sausage_pick_bounds("falling") == normal_bounds and Kitchen.sausage_pick_bounds("table") == normal_bounds, "Only the slipped state expands sausage interaction")
