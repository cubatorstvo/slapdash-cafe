...
func run()->void:
	...
	print("5/6: v20 persists the shared library")
	var saved: Dictionary=bytes_to_var(var_to_bytes(service.save_data()))
	check(saved.version==20 and saved.masterclasses.size()==1,"Current v20 save writes the masterclass library")
	check(service.load_data(saved),"v20 save reloads")
