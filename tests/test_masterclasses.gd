...
func run()->void:
	...
	print("5/6: v14 persists the shared library")
	var saved: Dictionary=bytes_to_var(var_to_bytes(service.save_data()))
	check(saved.version==14 and saved.masterclasses.size()==1,"Current save writes masterclass library v14")
	check(service.load_data(saved),"v14 save reloads")
