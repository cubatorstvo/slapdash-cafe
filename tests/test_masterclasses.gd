...
func run()->void:
	...
	print("5/6: v21 persists the shared library")
	var saved: Dictionary=bytes_to_var(var_to_bytes(service.save_data()))
	check(saved.version==21 and saved.masterclasses.size()==1,"Current v21 save writes the masterclass library")
	check(service.load_data(saved),"v21 save reloads")
