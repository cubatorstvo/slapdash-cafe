...
func run()->void:
	...
	print("5/6: v16 persists the shared library")
	var saved: Dictionary=bytes_to_var(var_to_bytes(service.save_data()))
	check(saved.version==16 and saved.masterclasses.size()==1,"Current save writes masterclass library v16")
	check(service.load_data(saved),"v16 save reloads")
