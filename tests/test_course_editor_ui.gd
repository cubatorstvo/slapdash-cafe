...
func run()->void:
	...
	check(game.office.schedule_editor.records==[2101,2102,2103],"UI T04 multi-selection becomes one ordered schedule draft")
	game.office._training_drop({"kind":"queue_lessons","schedule_id":0,"indices":[2]},{"zone":"draft","schedule_id":0,"index":0},false)
	check(game.office.schedule_editor.records==[2103,2101,2102],"UI T04 drag can reorder dishes before scheduling")
	game.office._training_drop({"kind":"queue_lessons","schedule_id":0,"indices":[0]},{"zone":"draft","schedule_id":0,"index":2},true)
	check(game.office.schedule_editor.records==[2101,2102,2103],"UI T04 drag can restore dish order")
	var preview: Dictionary=service.training_schedule_preview(game.office._draft_assignments(),"balanced",[])
	...
	await process_frame
	var views: Array=service.training_schedule_views()
	...
	check(await run_until(service,func():return str(service.training_queue._schedule(schedule_id).get("state",""))=="completed"),"UI T04 scheduled dishes complete")
	dispose(game)
	...
	game.office.schedule_editor_add_record(2201)
	preview=service.training_schedule_preview(game.office._draft_assignments(),"balanced",[])
	...
	await process_frame
	views=service.training_schedule_views()
	...
	game.office.schedule_editor_add_record(2301)
	...
	game.office._training_drop({"kind":"masterclass_records","record_ids":[2302,2303]},{"zone":"schedule","schedule_id":schedule_id,"index":0},false)
	await process_frame
	var edited_view: Dictionary=service.training_queue.schedule_view(schedule_id)
	...
	game.office._training_drop({"kind":"queue_lessons","schedule_id":schedule_id,"indices":[1]},{"zone":"schedule","schedule_id":schedule_id,"index":0},false)
	await process_frame
	edited_view=service.training_queue.schedule_view(schedule_id)
	...
	game.office.schedule_editor_add_record(2311)
	...
	game.office.schedule_editor_open(2312)
	game.office.schedule_editor_select_group(service.group_id_for_station(2),true)
	...
	var live_views: Array=service.training_schedule_views()
	...
	game.office.schedule_editor_add_record(2401)
	game.office.schedule_editor_add_record(2402)
	...
	game.office.schedule_editor_add_record(2491)
	...
	check(not game.office.schedule_editor.is_empty() and game.office.schedule_editor.records==[2491],"UI draft survives a rejected host confirmation")
	check(game.office.schedule_editor_message.contains("Подойди к компьютеру"),"UI draft shows the concrete host rejection reason")
	game.player.global_position=game.shop.computer.global_position
	check(press(game.office,"Добавить в расписание"),"UI draft can be confirmed after returning to the computer")
	await process_frame
	check(service.training_schedule_views().size()==1,"UI draft successful retry creates exactly one schedule")
	dispose(game)
	...
	check(game.office.tab=="groups" and game.office.groups_mode=="training" and game.office.schedule_editor.records==[2501] and str(game.office.schedule_editor.type_id)=="counter","Videotheque opens the training workspace with the record prefilled")
	dispose(game)
