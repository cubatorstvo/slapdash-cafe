...
func run()->void:
	...
	check(saved_t01.version==21 and saved_t01.table_group_registry.groups.size()>=2,"v21 stores explicit group registry")
	check(service.load_data(saved_t01),"v21 persistent groups reload")
	left=service.table_group_by_id(left_id)
	right=service.table_group_by_id(right_id)
