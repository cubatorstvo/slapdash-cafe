...
func run()->void:
	...
	preload("res://scripts/cafe_insights.gd").loss(service.analytics,"sausage","busy",1,0,[4,5],right_id,str(right.name))
	var saved_t01: Dictionary=bytes_to_var(var_to_bytes(service.save_data()))
	check(saved_t01.version==22 and saved_t01.table_group_registry.groups.size()>=2,"v22 stores explicit group registry")
	check(service.load_data(saved_t01),"v22 persistent groups reload")
	left=service.table_group_by_id(left_id)
	right=service.table_group_by_id(right_id)
	...
	check(service.table_group_by_id(right_id).is_empty(),"Empty organizational group is released after regrouping")
	var history_rows: Array=service.analytics.loss_details.values().filter(func(row):return str(row.get("group",""))==right_id)
	check(history_rows.size()==1 and str(history_rows[0].get("group_name",""))=="Правая линия","Historical event keeps the deleted group's display name")
	...
	check(not service.create_table_group([2,3],"Конфликтная линия").is_empty(),"Create-from-selected refuses to silently discard conflicting plans")
	...
	for merge_group_id in [four_id,str(split_after.id)]:
		service.set_group_dish_active(merge_group_id,"wine",false)
		service.set_group_dish_active(merge_group_id,"potato",false)
	check(service.merge_table_groups([four_id,str(split_after.id)],{"wine":800,"potato":901},[]).is_empty(),"Player-selected final assignments merge the groups with an explicitly empty menu")
	var merged: Dictionary=service.table_group_by_id(four_id)
	check(merged.active_dishes.is_empty(),"Merging groups can intentionally leave every dish disabled")
	...
	check(migrated_save.version==22 and migrated_save.table_group_registry.groups.has("2-3-4-5"),"Migrated cafe subsequently saves only the persistent v22 group identity")
