...
func run() -> void:
	...
	assert(saved.version == 16, "slot-based saves should use current format v16")
	...
	assert(game.service.save_data().version == 16, "migrated saves should write current format v16")
