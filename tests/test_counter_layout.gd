...
func run() -> void:
	...
	assert(saved.version == 13, "slot-based saves should use current format v13")
	...
	assert(game.service.save_data().version == 13, "migrated saves should write current format v13")
