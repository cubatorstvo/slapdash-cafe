...
func run() -> void:
	...
	assert(saved.version == 20, "slot-based saves should use current format v20")
	...
	assert(game.service.save_data().version == 20, "migrated saves should write current format v20")
