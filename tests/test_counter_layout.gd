...
func run() -> void:
	...
	var shelf = station.view.get_node("CounterStorage/ProductShelf")
	assert(shelf.visible, "authored product shelf should be the visible shelf")
	assert(is_equal_approx(shelf.rotation.y, m.Layout.SHELF_YAW), "visible authored product shelf should use the 45 degree rotation")
