...
func run() -> void:
	...
	assert(saved.version == 15, "slot-based saves should use current format v15")
	...
	assert(game.service.save_data().version == 15, "migrated saves should write current format v15")
