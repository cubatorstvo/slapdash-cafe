...
func run() -> void:
	...
	assert(saved.version == 21, "slot-based saves should use current format v21")
	...
	assert(game.service.save_data().version == 21, "migrated saves should write current format v21")
