...
func run() -> void:
	...
	assert(saved.version == 17, "slot-based saves should use current format v17")
	...
	assert(game.service.save_data().version == 17, "migrated saves should write current format v17")
