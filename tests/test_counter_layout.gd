...
func run() -> void:
	...
	assert(saved.version == 14, "slot-based saves should use current format v14")
	...
	assert(game.service.save_data().version == 14, "migrated saves should write current format v13")
