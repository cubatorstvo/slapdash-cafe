...
func run()->void:
	...
	check(saved.version==17,"Scaling save format is v17")
	check(service.load_data(saved),"v17 scaling cafe reloads")
	...
	check(service.save_data().version==17,"Migrated cafe writes the current v17 format")
