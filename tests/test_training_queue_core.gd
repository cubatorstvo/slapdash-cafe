...
func run()->void:
	...
	var course:=service.queue_training_course([
	...
	var view:=queue.course_view(course_id)
	...
	var stored:=queue._course(course_id)
	var batch:=queue._batch(int(stored.batch_ids[0]))
	...
	var before_courses:=queue.courses.size()
	...
	var equipment_course:=service.queue_training_course([{"record_id":1202,"station_ids":[7]}],"balanced","equipment",1)
