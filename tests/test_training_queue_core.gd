...
func run()->void:
	...
	print("T06: blocked batch does not stall an independent ready batch")
	stations[6].equipment.erase("pan")
	stations[6].apply_equipment()
	var t06_blocked: Dictionary=service.queue_training_course([{"record_id":1202,"station_ids":[8]}],"together","t06-blocked",1)
	var t06_ready: Dictionary=service.queue_training_course([{"record_id":1201,"station_ids":[9]}],"together","t06-ready",1)
	service.advance(0.1)
	await process_frame
	...
	stations[6].apply_equipment()
	check(await run_until(service,func():return course_state(queue,int(t06_blocked.course_id))=="completed"),"T06 blocked course starts automatically after pan installation")
