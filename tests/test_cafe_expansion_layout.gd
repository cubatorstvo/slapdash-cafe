...
func run()->void:
	...
	for slot in range(Expansion.SLOT_COUNT):
		...
		check(hall_contains(worker,stage),"Slot %d worker edge stays inside its unlock stage"%(slot+1))
		var rest_route:=Expansion.route_to_rear(worker,Annex.REST_DOOR_CAFE)
		check(route_inside(rest_route,stage),"Worker route from slot %d reaches the lounge door on opened floor"%(slot+1))
		var return_route:=rest_route.duplicate()
		return_route.reverse()
		check(route_inside(return_route,stage),"Worker return route to slot %d stays on opened floor"%(slot+1))
		if slot>0:
