...
func run() -> void:
	...
	check(goal.key == "fifth_prep_done", "Journey reaches the explicit final-inspection handoff")
	check(not p.can_attempt(service.stations, service.served), "Current code does not accidentally start an unfinished fifth-star inspection")
