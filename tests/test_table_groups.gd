...
func run()->void:
	...
	print("7/7: v16 persists group names, learned source links and working recipes")
	var saved: Dictionary=bytes_to_var(var_to_bytes(service.save_data()))
	check(saved.version==16 and saved.table_group_names.get(str(counter_group.id),"")=="Картофельная линия","Current save writes group metadata in v16")
	check(service.load_data(saved),"v16 cafe reloads")
