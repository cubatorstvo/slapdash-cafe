# P5.04 — граф развития и завершение кампании

База: `166b950cfb72c08e983bddb9f03437165ce58bdf` (main после P5.03). Движок: Godot `4.7.2.stable.official.ed1daf0bf`.

## Граница результата

Этот PR исправляет источники фактов, завершает кампанию на 5★ и передаёт точную спецификацию каталогов. Основной путь уже реализован в P5.01–P5.03. `P5_CATALOG_RULES.json` описывает **целевые правила для реализации P5.05**: 52 товара, 16 наборов, 4 расширения помещений и технические бесплатные инструменты. Сам JSON не исполняется игрой. Действующий код магазинов пока сохраняет прежние широкие наборы; Grok заменяет их согласно спецификации, а не проектирует условия заново.
...
|---|---|---|---|
| 1 | 15 ручных подач, освоены сосиска/картофель/вино | Дегустация трёх стартовых блюд, каждое B+; неудачное блюдо можно повторить | +120; базовая лаборатория |
...
Цены и награды сохранены. Порог подготовки и порог итогового события — разные проверки. Флаг `finish_banquet(true)` без требуемой фазы и фактических счётчиков не выдаёт 5★. Повторный вызов после результата не выдаёт награду и не переписывает итоги. Провал оставляет 4★; повтор бесплатен.
...
- **1★:** лаборатория исключена из требований и доступна только после награды. 15 ручных подач и три блюда доступны без клона.
