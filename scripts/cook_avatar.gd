...
func perform(pose: Dictionary, target: Vector3, holding: bool) -> void:
	...
	if book.visible:
		head.rotation.x = minf(head.rotation.x, -0.18)
		P.align_line(arms[0], Vector3(-0.3, 1.2, 0), Vector3(-0.42, 1.08, -0.38))
		P.align_line(arms[1], Vector3(0.3, 1.2, 0), Vector3(0.42, 1.08, -0.38))
