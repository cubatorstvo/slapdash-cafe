...
func _apply_feature_navigation() -> void:
	...
	for key in navigation:
		var show_in_sidebar: bool = key in ["overview","stations","star","groups","videos"]
		if key == "groups": show_in_sidebar = _feature_visible("staff_roster")
		elif key == "videos": show_in_sidebar = _feature_visible("video_recording")
		navigation[key].visible = show_in_sidebar
		if show_in_sidebar and NAV_LABELS.has(key): navigation[key].text = NAV_LABELS[key]
	var side := get_node("Panel/Column/Body/SideScroll/Side")
	var order_index := 0
	for key in ["overview","stations","star","groups","videos"]:
		if navigation.has(key) and navigation[key].visible:
			side.move_child(navigation[key], order_index)
			order_index += 1
...
func _process(delta: float) -> void:
	if game!=null and is_instance_valid(game.get("service")) and opened():
		...
		var feature_revision: int = service.feature_access.revision() if service.get("feature_access") != null else 0
...
func _product_row(parent: Node, item: String, station_id: int, installed: bool, reason: String, selection := false) -> HBoxContainer:
	...
	if bool(state.get("visible", true)) and not bool(state.get("enabled", true)) and final_reason.is_empty() and access != null: final_reason = access.reason_text(state)
...
func _decorate_development() -> void:
	var access = _feature_access()
	if access == null: return
	if game.service.progress.manual_served <= 0:
...
func _nearest_teaser() -> String:
	var access = _feature_access()
	if access == null: return ""
	var preview: Dictionary = access.next_unlock_preview()
	if preview.is_empty(): return ""
	var title := str(preview.get("title", ""))
	var condition := str(preview.get("condition", ""))
	return title if condition.is_empty() else "%s · %s" % [title, condition]

func _decorate_staff() -> void:
	var card := _section_card(content, Color("294647"))
	label(card, "РАЗВИТИЕ СОТРУДНИКОВ", 13)
	if game.service.clone_options().is_empty(): label(card, "Сейчас сотрудников нет. Уже открытый раздел не исчезает — вырасти нового клона в лаборатории.", 14)
...
func _open_overview_problem(problem: Dictionary) -> void:
	...
	if reason=="equipment":
		...
		open_shop("equipment")
	elif reason=="workers": open_problem_page("laboratory")
	elif reason=="no_station": open_shop("tables")
	elif reason in ["busy","unlearned","training","menu_off","wait"] and not stations.is_empty(): open_problem_group(stations)
	else: navigate("stats")
...
func overview_page(_host: bool) -> void:
	...
	if not previous.is_empty() and int(previous.get("day",0))!=p.day: label(summary,"Предыдущая смена · выручка %d · обслужено %d · ушли %d"%[int(previous.get("revenue",0)),int(previous.get("served",0)),int(previous.get("missed",0))],13)
	var production: Dictionary=service.production_overview()
	...
	visit_card()
	if _feature_visible("cafe_statistics"):
